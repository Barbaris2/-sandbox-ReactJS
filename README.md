...
- `git checkout 05_lazy` - **React.lazy()** - метод из API - React suspense, который может грузить компоненты асинхронно. Позволяет определять компонент, который загружается динамически.
